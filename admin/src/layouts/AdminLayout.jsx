...
import {
  Menu as MenuIcon, Dashboard, People, LocalHospital, Medication, Science,
  AttachMoney, Settings, Logout, Notifications, ChevronLeft, Person,
  AdminPanelSettings, CalendarMonth, Assessment, LocationCity, LightMode,
  DarkMode, Search as SearchIcon
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            <Box>
              ...
              <Typography variant="caption" sx={{ color: alpha('#fff', 0.7) }}>Admin Portal</Typography>
            </Box>
          </Stack>
          {!isMobile && (
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <AppBar position="fixed" elevation={0} sx={{ zIndex: theme.zIndex.drawer + 1, backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0, 0, 0, 0.08)' }}>
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          <Stack direction="row" alignItems="center" spacing={2}>
            <IconButton onClick={handleDrawerToggle} sx={{ color: '#1a237e' }}><MenuIcon /></IconButton>
            <Box>
              <Typography variant="h6" sx={{ color: '#1a237e', fontWeight: 700, fontSize: { xs: 16, sm: 20 } }}>Hospital Management System</Typography>
              <Typography variant="caption" sx={{ color: alpha('#1a237e', 0.6) }}>Welcome back, Admin</Typography>
            </Box>
          </Stack>
          <Stack direction="row" alignItems="center" spacing={1}>
            <Tooltip title="Toggle theme">
              <IconButton onClick={() => setDarkMode(!darkMode)} sx={{ color: '#1a237e' }}>
                {darkMode ? <LightMode /> : <DarkMode />}
              </IconButton>
            </Tooltip>
            <Tooltip title="Notifications">
              <IconButton sx={{ color: '#1a237e' }}>
                <Badge badgeContent={4} sx={{ '& .MuiBadge-badge': { backgroundColor: '#f44336', color: 'white' } }}><Notifications /></Badge>
              </IconButton>
            </Tooltip>
            <Tooltip title="Account settings">
              <IconButton onClick={handleMenuOpen} sx={{ ml: 1 }}>
                <Avatar sx={{ width: 36, height: 36, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', fontWeight: 700, fontSize: 14 }}>AD</Avatar>
              </IconButton>
            </Tooltip>
          </Stack>
      ...
      <Paper elevation={0} sx={{ position: 'fixed', bottom: 0, left: { sm: open ? drawerWidth : 0 }, right: 0, py: 1.5, px: 3, backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(0, 0, 0, 0.08)', zIndex: theme.zIndex.drawer - 1, transition: theme.transitions.create(['left']) }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems="center" spacing={1}>
          <Typography variant="body2" color="text.secondary" fontSize={13}>© {new Date().getFullYear()} Hospital Management System</Typography>
          <Stack direction="row" spacing={2}>
            <Chip label="v2.0.0" size="small" sx={{ height: 22, fontSize: 11, fontWeight: 600, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }} />
